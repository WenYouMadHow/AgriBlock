...
main = Tk()
main.title("Securing Agricultural Supply Chain Using Blockchain")
...
if os.path.exists('blockchain_contract2.txt'):
    with open('blockchain_contract2.txt', 'rb') as fileinput:
...
def saveASCDetails():
    ...
    if len(pid) > 0 and len(pname) > 0 and len(desc) > 0:
        ...
        blockchain.save_object(blockchain,'blockchain_contract2.txt')
        tf1.delete(0, END)
        tf2.delete(0, END)
        tf3.delete(0, END)
        end = time.time()
...
font = ('roboto', 18, 'bold')
title = Label(main, text='Securing Agricultural Supply Chain Using Blockchain')
title.config(bg='#f6e58d', fg='#30336b')  
title.config(font=font)           
title.config(height=3, width=50)       
title.place(x=380,y=10)

font1 = ('roboto', 13, 'bold')
...
# Set initial values for the entries and user type
tf1.insert(0, "1")
tf2.insert(0, "Sample Product")
tf3.insert(0, "Sample Description")
userlist.set("Providers")

# Call the saveASCDetails function to save the initial details
saveASCDetails()

main.config(bg='#1289A7')
main.mainloop()